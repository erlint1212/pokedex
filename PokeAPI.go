...
func getPokemon(c *pokecache.Cache, url string) (PokemonEndpoint, error) {
    if data, OK := c.Get(url); OK {
        fmt.Println("Found in cache!")
        var poke_end PokemonEndpoint
        if err := json.Unmarshal(data, &poke_end); err != nil {
            return PokemonEndpoint{}, err
        }
        return poke_end, nil
    }
	res, err := http.Get(url)
